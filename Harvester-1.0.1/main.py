    ...
    for failedVerify in range(100):
        ...
        try:
            if captcha_type == "ReCaptcha V2":
                request = function_driver.wait_for_request(path='/recaptcha/api2/userverify', timeout=10000)

            if captcha_type == "ReCaptcha V3":
                if failedVerify != 0:
                    time.sleep(2)
                    function_driver.refresh()
                request = function_driver.wait_for_request(path='/recaptcha/api2/reload', timeout=10000)
    ...
    try:
        ...
        for usedName in range(100):
            ...
            try:
                ...
                browser_storage = browser_storage_beta.replace('Harvester-1.0.1', 'Browser-Profiles\\')
        ...
        login_driver = webdriver.Chrome(options=profile_arguments.opts, seleniumwire_options=set_proxy.proxy_options,
                                        executable_path="C:\\Users\\jasta\\PROGRAMMING\\TETRA AIO\\main\\ReCaptcha\\Harvester-1.0.1\\chromedriver.exe")
        print(colors.CYAN + "Please enter your login information." + colors.END)
        login_driver.get('https://gmail.com')
        try:
            login_wait = login_driver.wait_for_request(path='mail/u/0', timeout=100)
        except TimeoutError:
            os.system('cls')
            login_driver.close()
            print(colors.FAIL + 'Tetra Harvester timed out. Please restart and try again.' + colors.END)
...
def captcha(site, captcha_type, token_count):
    ...
    try:
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        browser_storage = dir_path.replace('Harvester-1.0.1', 'Browser-Profiles\\')
        ...
        browser_storage_base = dir_path.replace('Harvester-1.0.1', 'Browser-Profiles\\')
        browser_storage = browser_storage_beta.replace('Harvester-1.0.1', 'Browser-Profiles\\')
        ...
        if profile_found:
            ...
            profile_arguments.opts.add_argument('user-data-dir=' + browser_storage)
            driver = webdriver.Chrome(seleniumwire_options=set_proxy.proxy_options, executable_path='chromedriver.exe',
            ...
            driver.wait_for_request(path='mail/u/0', timeout=100)
...
def profiles():
    try:
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        browser_storage = dir_path.replace('Harvester-1.0.1',
                                           'Browser-Profiles\\')  # C:\Users\jasta\PROGRAMMING\TETRA AIO\main\ReCaptcha\Browser-Profiles
        browser_profile_list = os.listdir(browser_storage)
        browser_profile_list.append(colors.CYAN + 'Main Menu' + colors.END)
        browser_profile_list.append(colors.CYAN + 'Exit' + colors.END)
